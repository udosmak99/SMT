...
Partial Class _Default
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Dim StationInfo(tblStn.Rows.Count - 1, 29) As Object
        Dim CountStation(0, 5) As Object
        Dim stationdata As New Generic.List(Of clsLastdata)
        Dim url As String

        Dim i As Integer
        Dim scriptjson As String = " var storeInfo = [ " & vbCrLf
        ...
        Try
            ...
            For Each dtrow In tblStn.Rows
                ...
                If Not dtrow.Item("server_time_up") Is DBNull.Value Then
                    stndata.servertime_up = CStr(GolbalFunc.Validation.NulltoString(GolbalFunc.ThaiDatetime.ThaiFormat(dtrow.Item("server_time_up"), "วันddddที่ dd MMMM yyyy")))
                Else
                    stndata.servertime_up = CStr(GolbalFunc.Validation.NulltoString(dtrow.Item("server_time_up")))
                End If
                ...
                Else
                    StationInfo(i, 4) = (CDbl(stndata.wl_up)).ToString("#,##0.00")
                End If
                If stndata.wl_down = "-" Then
                    StationInfo(i, 5) = stndata.wl_down
                Else
                    StationInfo(i, 5) = (CDbl(stndata.wl_down)).ToString("#,##0.00")
                End If
                If stndata.rf15 = "-" Then
                    StationInfo(i, 6) = stndata.rf15
                Else
                    StationInfo(i, 6) = (CDbl(stndata.rf15)).ToString("#,##0.00")
                End If
                If stndata.rf_7to7 = "-" Then
                    StationInfo(i, 7) = stndata.rf_7to7
                Else
                    StationInfo(i, 7) = (CDbl(stndata.rf_7to7)).ToString("#,##0.00")
                End If
                If stndata.rf_7tonow = "-" Then
                    StationInfo(i, 8) = stndata.rf_7tonow
                Else
                    StationInfo(i, 8) = (CDbl(stndata.rf_7tonow)).ToString("#,##0.00")
                End If
                ...
                StationInfo(i, 27) = stndata.servertime_up
                StationInfo(i, 28) = stndata.address
                StationInfo(i, 29) = i + 1
                scriptjson &= "{ " & vbCrLf
                scriptjson &= " code: """ & stndata.station_id & """, " & vbCrLf
                scriptjson &= " code_customer: """ & stndata.station_code & """, " & vbCrLf
                scriptjson &= " name: """ & stndata.station_name & """, " & vbCrLf
                scriptjson &= " address: """ & stndata.station_name & """, " & vbCrLf
                scriptjson &= " group: '" & stndata.group_id & "', " & vbCrLf
                scriptjson &= " group_name: """ & stndata.group_name & """, " & vbCrLf
                scriptjson &= " province_id: """ & stndata.province_id & """, " & vbCrLf
                scriptjson &= " province_name: """ & stndata.province_name & """, " & vbCrLf
                scriptjson &= " basin_id: """ & stndata.basin_id & """, " & vbCrLf
                scriptjson &= " basin_name: """ & stndata.basin_name & """, " & vbCrLf
                scriptjson &= " water: '" & seticon & "', " & vbCrLf
                scriptjson &= " rain: '" & StationInfo(i, 6) & "', " & vbCrLf
                scriptjson &= " rain7to7: '" & StationInfo(i, 7) & "', " & vbCrLf
                scriptjson &= " rain7tonow: '" & StationInfo(i, 8) & "', " & vbCrLf
                scriptjson &= " lat: '" & stndata.lat & "', " & vbCrLf
                scriptjson &= " long: '" & stndata.longtitude & "', " & vbCrLf
                scriptjson &= " gatecount: '" & stndata.gatecount & "', " & vbCrLf
                scriptjson &= " linkurl: """ & url & """, " & vbCrLf
                scriptjson &= " dateupdate: """ & stndata.servertime_up & """, " & vbCrLf
                scriptjson &= " index: """ & i + 1 & """, " & vbCrLf
                scriptjson &= " water_level: " & vbCrLf
                scriptjson &= " { " & vbCrLf
                scriptjson &= " value1: '" & StationInfo(i, 4) & "', " & vbCrLf
                scriptjson &= " value2: '" & StationInfo(i, 5) & "', " & vbCrLf
                scriptjson &= " value3: '" & StationInfo(i, 16) & "' " & vbCrLf
                If stndata.gatecount = 0 Then
                    scriptjson &= " } " & vbCrLf
                Else
                    scriptjson &= " }, " & vbCrLf
                End If

                If stndata.gatecount = 1 Then
                    scriptjson &= " door_level: [ " & vbCrLf
                    scriptjson &= " { " & vbCrLf
                    scriptjson &= " value: '" & stndata.gate1 & "' " & vbCrLf
                    scriptjson &= " } " & vbCrLf
                    scriptjson &= " ] " & vbCrLf
                ElseIf stndata.gatecount = 2 Then
                    scriptjson &= " door_level: [ " & vbCrLf
                    scriptjson &= " { " & vbCrLf
                    scriptjson &= " value: '" & stndata.gate1 & "' " & vbCrLf
                    scriptjson &= " } " & vbCrLf
                    scriptjson &= " ,{ " & vbCrLf
                    scriptjson &= " value: '" & stndata.gate2 & "' " & vbCrLf
                    scriptjson &= " } " & vbCrLf
                    scriptjson &= " ] " & vbCrLf
                ElseIf stndata.gatecount = 3 Then
                    scriptjson &= " door_level: [ " & vbCrLf
                    scriptjson &= " { " & vbCrLf
                    scriptjson &= " value: '" & stndata.gate1 & "' " & vbCrLf
                    scriptjson &= " } " & vbCrLf
                    ...
                    scriptjson &= " } " & vbCrLf
                    scriptjson &= " ,{ " & vbCrLf
                    scriptjson &= " value: '" & stndata.gate3 & "' " & vbCrLf
                    scriptjson &= " } " & vbCrLf
                    scriptjson &= " ] " & vbCrLf
                ElseIf stndata.gatecount = 4 Then
                    scriptjson &= " door_level: [ " & vbCrLf
                    scriptjson &= " { " & vbCrLf
                    scriptjson &= " value: '" & stndata.gate1 & "' " & vbCrLf
                    scriptjson &= " } " & vbCrLf
                    ...
                    scriptjson &= " } " & vbCrLf
                    scriptjson &= " ,{ " & vbCrLf
                    scriptjson &= " value: '" & stndata.gate4 & "' " & vbCrLf
                    scriptjson &= " } " & vbCrLf
                    scriptjson &= " ] " & vbCrLf
                ElseIf stndata.gatecount = 5 Then
                    scriptjson &= " door_level: [ " & vbCrLf
                    scriptjson &= " { " & vbCrLf
                    scriptjson &= " value: '" & stndata.gate1 & "' " & vbCrLf
                    scriptjson &= " } " & vbCrLf
                    ...
                    scriptjson &= " } " & vbCrLf
                    scriptjson &= " ,{ " & vbCrLf
                    scriptjson &= " value: '" & stndata.gate5 & "' " & vbCrLf
                    scriptjson &= " } " & vbCrLf
                    scriptjson &= " ] " & vbCrLf
                ElseIf stndata.gatecount = 6 Then
                    scriptjson &= " door_level: [ " & vbCrLf
                    scriptjson &= " { " & vbCrLf
                    scriptjson &= " value: '" & stndata.gate1 & "' " & vbCrLf
                    scriptjson &= " } " & vbCrLf
                    ...
                    scriptjson &= " } " & vbCrLf
                    scriptjson &= " ,{ " & vbCrLf
                    scriptjson &= " value: '" & stndata.gate6 & "' " & vbCrLf
                    scriptjson &= " } " & vbCrLf
                    scriptjson &= " ] " & vbCrLf
                ElseIf stndata.gatecount = 7 Then
                    scriptjson &= " door_level: [ " & vbCrLf
                    scriptjson &= " { " & vbCrLf
                    scriptjson &= " value: '" & stndata.gate1 & "' " & vbCrLf
                    scriptjson &= " } " & vbCrLf
                    ...
                    scriptjson &= " } " & vbCrLf
                    scriptjson &= " ,{ " & vbCrLf
                    scriptjson &= " value: '" & stndata.gate7 & "' " & vbCrLf
                    scriptjson &= " } " & vbCrLf
                    scriptjson &= " ] " & vbCrLf
                End If
                If i = tblStn.Rows.Count - 1 Then
                    scriptjson &= "}" & vbCrLf
                Else
                    scriptjson &= "}," & vbCrLf
                End If
            ...
            ClientScript.RegisterStartupScript(Me.GetType(), "uploadNotify", "<script " + "type='text/javascript'> " + scriptjson + "<" + "/script" + ">")
